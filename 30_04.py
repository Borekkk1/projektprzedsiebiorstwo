...
class MagazynApp:
    ...
    def create_raporty_tab(self):
        ...
        # --- NOWE PRZYCISKI ZADAŃ 2, 3, 4, 5 ---
        ttk.Button(ctrl_frame, text="5. Niski stan", command=self.raport_niski_stan).pack(side="left", padx=5)
        ttk.Button(ctrl_frame, text="6. Ranking ruchu", command=self.raport_ranking_ruchu).pack(side="left", padx=5)
        ttk.Button(ctrl_frame, text="7. Trendy miesięczne", command=self.raport_trendow).pack(side="left", padx=5)
        ttk.Button(ctrl_frame, text="8. Obrót materiału", command=self.raport_obrotow_materialu).pack(side="left", padx=5)
    ...
    # --- IMPLEMENTACJA ZADAŃ 2, 3, 4 i 5 ---

    def raport_niski_stan(self):
        """Zadanie 2: Materiały o niskim stanie (<10)"""
        self.clear_report_table()
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("""
            SELECT m.Nazwa, mag.Kod,
                   SUM(CASE WHEN o.TypOperacji='Przyjcie' THEN o.Ilo ELSE 0 END) as SumaPrzyjec,
                   SUM(CASE WHEN o.TypOperacji='Wydanie' THEN o.Ilo ELSE 0 END) as SumaWydan,
        ...
        """)
        for lp, row in enumerate(cur.fetchall(), 1):
            nazwa, mag, p, w, stan = row
            self.raporty_tree.insert("", "end", values=(lp, nazwa, mag, stan, "-", f"Przyjęcia: {p}, Wydania: {w}"))
        conn.close()

    def raport_ranking_ruchu(self):
        """Zadanie 3: Ranking największego ruchu (suma P + suma W)"""
        self.clear_report_table()
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("""
            SELECT m.Nazwa, 
                   COUNT(o.OperacjaID) as LiczbaOp,
                   SUM(CASE WHEN o.TypOperacji='Przyjcie' THEN o.Ilo ELSE 0 END) as SumaP,
                   SUM(CASE WHEN o.TypOperacji='Wydanie' THEN o.Ilo ELSE 0 END) as SumaW,
                   SUM(o.Ilo) as LacznyRuch
            FROM OperacjeMagazynowe o
            JOIN Materialy m ON o.MaterialID = m.MaterialID
            GROUP BY m.MaterialID
            ORDER BY LacznyRuch DESC
        """)
        for lp, row in enumerate(cur.fetchall(), 1):
            nazwa, lic_op, p, w, ruch = row
            self.raporty_tree.insert("", "end", values=(f"Poz.{lp}", nazwa, "-", ruch, lic_op, f"P: {p}, W: {w}"))
        conn.close()

    def raport_trendow(self):
        """Zadanie 4: Raport trendów miesięcznych z wykresem"""
        self.clear_report_table()
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("""
            SELECT strftime('%Y-%m', o.DataOperacji) as Miesiac,
                   SUM(CASE WHEN o.TypOperacji='Przyjcie' THEN o.Ilo ELSE 0 END) as P,
                   SUM(CASE WHEN o.TypOperacji='Wydanie' THEN o.Ilo ELSE 0 END) as W
            FROM OperacjeMagazynowe o
            WHERE o.DataOperacji IS NOT NULL
            GROUP BY Miesiac
        ...
        """)
        rows = cur.fetchall()
        for lp, row in enumerate(rows, 1):
            mies, p, w = row
            self.raporty_tree.insert("", "end", values=(lp, "TREND ZBIORCZY", "-", "-", "-", f"{mies}: P={p}, W={w}"))
        
        if MATPLOTLIB_OK and rows:
            self.clear_chart()
            x = [r[0] for r in rows]
            y_p = [float(r[1]) for r in rows]
            y_w = [float(r[2]) for r in rows]
            
            fig = Figure(figsize=(8.8, 3.6), dpi=100)
            ax = fig.add_subplot(111)
            ax.plot(x, y_p, label="Przyjęcia", marker='o', color='blue')
            ax.plot(x, y_w, label="Wydania", marker='s', color='red')
            ax.legend()
            ax.set_title("Trend miesięczny operacji")
    ...
    def raport_obrotow_materialu(self):
        """Zadanie 5: Raport obrotów dla wybranego materiału (z comboboxa Przyjęcia)"""
        material_nazwa = self.przyjecia_material_combo.get()
        if not material_nazwa:
            messagebox.showwarning("Wybór", "Proszę najpierw wybrać materiał w zakładce 'Przyjęcia'.")
            return
        ...
        for lp, row in enumerate(rows, 1):
            mies, p, w, obrot = row
            self.raporty_tree.insert("", "end", values=(lp, material_nazwa, "-", obrot, "-", f"{mies}: P={p}, W={w}"))
        ...
        if MATPLOTLIB_OK and rows:
            ...
            ax.set_title(f"Miesięczny obrót: {material_nazwa}")
